# File names
Get-ChildItem -Path . -Recurse -exclude *.dll -erroraction 'silentlycontinue' -Filter "*pwds*" -File | % {Write-Host $_.FullName} >> WinPwds.txt
Get-ChildItem -Path . -Recurse -exclude *.dll -erroraction 'silentlycontinue' -Filter "*pass*" -File | % {Write-Host $_.FullName} >> WinPwds.txt
Get-ChildItem -Path . -Recurse -exclude *.dll -erroraction 'silentlycontinue' -Filter "*login*" -File | % {Write-Host $_.FullName} >> WinPwds.txt
Get-ChildItem -Path . -Recurse -exclude *.dll -erroraction 'silentlycontinue' -Filter "*mdp*" -File | % {Write-Host $_.FullName} >> WinPwds.txt
